...
| Frontend       | Next.js (`frontend/nextjs`) + static HTML/JS fallback (`frontend/static`) |
...
# 2. Ingest PDFs
python ingestion/ingest.py --input /path/to/pdfs

# 3. Start API
cd backend && python api.py          # FastAPI at :8000

# 4. Query via CLI
cd backend && python main.py --query "principles of natural justice?"
```
